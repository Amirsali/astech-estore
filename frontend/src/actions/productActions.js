...
// getting product details by id from backend and update redux store with it
  ...
  try {
    ...
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: PRODUCT_DELETE_FAILED, payload: message });
  }
};
